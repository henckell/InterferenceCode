...
#   results_plot(typeofgraphs[j])
# }
# 
# 
# results_plot <- function(typofgraph){
...
n <- rep(nvals,4)
# color_var <- as.factor(c(rep(1,length(nvals)),rep(2,length(nvals)),rep(3,length(nvals)),rep(4,length(nvals)),rep(4,length(nvals))))
...
data_bias <- data.frame(Bias=c(bias_ols1,bias_ols2,bias_ols3,bias_ols4),
                        Estimator=Estimator,N=rep(nvals,4))
data_var <- data.frame(Variance=c(var_ols1,var_ols2,var_ols3,var_ols4),
                        Estimator=Estimator,
                       N=rep(nvals,4))
...
g_var <- ggplot(data = data_var, aes(x = log(N), y = log(Variance))) +  ylim(-8.5,0) + xlim(5.2,8.8) +
  geom_point(aes(shape = Estimator)) + theme(legend.position = "none",legend.key.size = unit(1, 'cm')) + 
  geom_line(aes(linetype=Estimator))
